...
modelname=f"FFT_corrRots_Norm_LR_mrsi_bs500_bpe2_snrNone_lr_0.0001" # No need for FFT?
...
#torch.save(mrsi_recons, "FFT_mrsi_recons.pt")
...
plot_mrsi_spectra(mrsi_fs, mrsi_us, mrsi_recons, i=i, j=j, k=k)


T=8
t=50
us_images = np.abs(mrsi_us[:, :, slice_index, t, :T])
fs_images = np.abs(mrsi_fs[:, :, slice_index, t, :T])
recons_images = np.abs(mrsi_recons[:, :, slice_index, t, :T])
...
T=7
t=8
us_images = np.abs(mrsi_us[:,:,slice_index,  45:53, T])
fs_images = np.abs(mrsi_fs[:,:,slice_index,  45:53, T])
recons_images = np.abs(mrsi_recons[:,:,slice_index,  45:53, T])
